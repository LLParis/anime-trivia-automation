...
Version 0.9.1 fixes the initial account-fallback launch failure: the signed CLI now lives in a versioned shared-tools location instead of Codex's app-virtualized LocalAppData, and startup pins its verified SHA-256. The supported Antigravity headless CLI is a bounded text-only fallback under the workstation's cached Google account, using Google account/AI-plan quota instead of silently retrying the exhausted Gemini Developer API key. It runs only after all available primary resolvers abstain or are unavailable; it never becomes a second Gemini vote against Qwen, and real provider disagreement still abstains. Each call has a six-second queue-plus-execution deadline, a fresh empty working directory, a minimal environment allowlist, disabled slash commands, sandboxing, bounded output, and Windows Job Object ownership so F12 kills the exact process tree before shutdown waits.
...
The installed workstation config enables the `novel` lane and points at its verified Qwen3.8-27B Q6 GGUF plus pinned llama.cpp CUDA runtime. It also enables Gemini 3.7 when `GEMINI_API_KEY` is present in the user environment; the key is never stored in JSON or the repository. The workstation additionally enables the signed Antigravity 1.1.23 CLI at `D:\11_CS\00_TOOLS\antigravity-cli\1.1.23\agy.exe`; its SHA-256 is pinned in config. Portable installs can use `%LOCALAPPDATA%\agy\bin\agy.exe` after account authentication. Run `agy models` once and confirm `gemini-3.7-flash-low` is listed before enabling `antigravity`. The older in-process `vlm` lane remains disabled.
